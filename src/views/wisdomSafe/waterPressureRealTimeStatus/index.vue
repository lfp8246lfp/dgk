  ...
  <div id="realtimecontrol">
      ...
      <ul class="device_body_lists">
        ...
        <li class="item">
          ...
          <p>
            ...
            <span :class="{green: item.curStatus === 1, grey: item.curStatus}">{{item.curStatus | deviceStaus}}</span>
          </p>
          <p>
    ...
    <el-dialog
      ...
      <div>
        ...
        <el-button type="primary" @click.native="searchSwitch">{{$t('common.query')}}</el-button>
        ...
        <div>
          <el-table
            ...
            <el-table-column
              props=""
              :label="$t('historicaldata.devOrder')">
              <template slot-scope="scope">{{pointComm}}</template>
            </el-table-column>
            <el-table-column 
              prop="" 
              :label="$t('historicaldata.deviceName')">
              <template slot-scope="scope">{{pointName}}</template>
            </el-table-column>
            <el-table-column 
              prop="dataTime" 
              :label="$t('historicaldata.dataTime')"
              width="180"
              :formatter="dateFormat">
            </el-table-column>
            <el-table-column 
              prop="smokeFlag" 
              :label="$t('historicaldata.alarmStatus')">
...
export default {
  ...
  methods:{
    ...
    getDeviceData() {
      this.$request('queryDeviceRealTimeStatus', {params: this.queryParams}).then(res => {
        if (res.data.returnCode === 0) {
          console.log('getDeviceData111', res)
    ...
    dateFormat(row, column, cellValue, index) {
      return dateformat(row[column.property], 'YYYY-MM-DD HH:mm')
    },
    hisSizeChange(size) {
      this.queryHistoryParams.pageSize = size
      this.searchSwitch()
    },
    hisCurrentChange(page) {
      this.queryHistoryParams.pageNum = page
      this.searchSwitch()
    }
  },
  computed: {
