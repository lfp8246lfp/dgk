    ...
    <el-button type="primary" @click="searchSwitch">{{$t('common.query')}}</el-button>
        <div>
            <el-table :data="tableData1" style="width: 100%;margin-top: 20px;" border row-key="index">
                ...
                <el-table-column prop="disc" :label="$t('historicaldata.deviceName')" >
                     <!-- lfp<template slot-scope="scope">{{deviceDics}}</template> -->
                </el-table-column>
                <el-table-column prop="datatime" :label="$t('historicaldata.dataTime')" :formatter="dateFormat"></el-table-column>
                <el-table-column prop="totalvalue" :label="$t('historicaldata.waterValue')"></el-table-column>
...
export default {
    ...
    methods: {
        ...
        check (index, row) {
         ...
         getDeviceData () {
            this.$request('queryApptypeList', {params: this.queryParams}).then(res => {
                if (res.data.returnCode === 0) {
                    console.log('queryApptypeList', res)
                    this.tableData1 = res.data.pointInfos
                    this.switchValOptions = res.data.pointInfos
                    this.total = res.data.total
                    this.switchValue = res.data.pointInfos[0].pointId
                }
            })
        },
        ...
        // 查询开关历史数据
        searchSwitch(){
            // this.queryHistoryParams.rtuId =  this.rtuId; //有历史数据
            // this.queryHistoryParams.pointid = Number(this.switchValue);
            // //lfp this.queryHistoryParams.datatype = Number(this.devValue);
        ...
        sendPost(){
            this.$request('queryWaterData',this.queryHistoryParams).then( res => {
                console.log("查询历史数据参数", this.queryHistoryParams)
                    console.log("查询历史数据res",res)
                    this.tableData1 = res.data.dcmWaters;
